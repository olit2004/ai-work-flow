# -*- coding: utf-8 -*-
"""Olit_oljira_kenea.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1yn9akyUznWvxsgBDyLfP3GxVlwHWS-3w
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# CONCRETE EXAMPLE — Permission quantale
# ─────────────────────────────────────────────────────────────────────────────
...
def build_quantflow_quantale() -> ResiduatedQuantale:
    ...
    Meaning:
      Combining two workflow guarantees yields the stricter / safer stage.
...
def demo() -> None:
    print("=" * 60)
    print("  Quantale — AI Content Pipeline Demo") # Updated description
    print("=" * 60)
    ...
    print("\n── Order queries ────────────────────────────────────────")
    # Updated pairs to match new Q elements
    pairs = [("raw","published"), ("researched","structured"), ("drafted","raw")]
    for a, b in pairs:
    ...
    # Updated pairs2 to match new Q elements
    pairs2 = [("researched","structured"), ("structured","drafted"), ("researched","drafted"), ("raw","drafted")]
    for a, b in pairs2:
        print(f"  {a} ⋁ {b} = {q.join(a,b)},   {a} ⋀ {b} = {q.meet(a,b)}")

    print("\n── Arbitrary joins (complete lattice) ───────────────────")
    # Updated subsets to match new Q elements
    subsets = [
    ...
    for k, v in report.items():
        if k != "counterexamples":
            print(f"  {k}: {v}")
    # Updated compositions to match new Q elements
    compositions = [("drafted","researched"), ("structured","drafted"), ("published","drafted"), ("researched","researched")]
    ...
    # ── Stage 8: residuals ────────────────────────────────────────────────────
    print("\n── Residuals (right: a → c) ─────────────────────────────")
    # Updated queries to match new Q elements and context
    queries = [
        ("drafted", "researched",   "max delegation: drafted content, budget=researched"),
        ("researched",  "drafted",  "max delegation: researched content, budget=drafted"),
        ("published", "structured", "max delegation: published content, budget=structured"),
        ("drafted", "drafted",  "max delegation: drafted content, budget=drafted"),
    ]
    for a, c, desc in queries:
        res = q.right_residual(a, c)
        print(f"  {a} → {c} = {res:12s}  ({desc})") # Adjusted padding
    ...
    # ── High-level API ────────────────────────────────────────────────────────
    print("\n── High-level API ───────────────────────────────────────")
    # Updated API calls to match new Q elements
    print("  can_do(drafted, researched):", q.can_do("drafted", "researched"))
    print("  can_do(researched, drafted):", q.can_do("researched", "drafted"))
    print("  effective_permission([researched,structured]):", q.effective_permission(["researched","structured"]))
    print("  max_delegatable(drafted, researched):        ", q.max_delegatable("drafted","researched"))
    print("  compose(drafted, researched):                ", q.compose("drafted","researched"))

    print("\n" + "=" * 60)
    print("  AI Content Pipeline demo complete.") # Updated description
    print("=" * 60)
